...
class SearchService:
    ...
    @staticmethod
    def search_persons(db: Session, query: str, limit: int = 10) -> List[PersonSearchResult]:
        """
        ...
        person_subquery = db.query(
            ...
            func.row_number().over(
                partition_by=Person.person_id,
                order_by=Participant.year.desc()
            ).label("row_number")
        ...
        results = db.query(
            ...
            person_subquery.c.row_number == 1
        ).limit(limit).all()

        person_results = []
        for result in results:
            # Build metadata: "Wrestler at Iowa State (2023, 165 lbs)"
            current_role = result.role_type.title() if result.role_type else "Participant"
            ...
            person_results.append(PersonSearchResult(
    ...
    @staticmethod
    def search_schools(db: Session, query: str, limit: int = 10) -> List[SchoolSearchResult]:
        """Search for schools"""
        search_term = f"%{query.lower()}%"
        
        results = db.query(School).filter(School.name.ilike(search_term)).limit(limit).all()
        ...
        for school in results:
            ...
            school_results.append(SchoolSearchResult(
    ...
    @staticmethod
    def search_tournaments(db: Session, query: str, limit: int = 10) -> List[TournamentSearchResult]:
        """Search for tournaments"""
        search_term = f"%{query.lower()}%"
        
        results = db.query(Tournament).filter(Tournament.name.ilike(search_term)).limit(limit).all()
        ...
        for tournament in results:
            ...
            tournament_results.append(TournamentSearchResult(
                tournament_id=tournament.tournament_id,
                name=tournament.name,
                primary_display=tournament.name,
                metadata=" • ".join(metadata_parts) if metadata_parts else "Tournament"
            ))
